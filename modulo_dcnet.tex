...
\begin{enumerate}
	...
	\item Los significados de diccPaquetesDCNet contienen un itConj que apunta al
		...
		encuentra
	\item Si colaPaquetesDCNet no es vacía, su próximo es un puntero a un
		paqueteDCNet de diccPaquetesDCNet que contiene un itConj cuyo siguiente
...
\Rep[estr][e]{
	...
	*($cd$.pc) = compuQueMásEnvió($e$.vectorCompusDCNet) $\land$ $e$.laQueMásEnvió = puntero($cd$)) $\yluego$\\
	...
	\- \- ($\forall cd_2$: compuDCNet)(está?($cd_2$, $e$.vectorCompusDCNet) $\implies$ \\
	...
	\- \- ($\emptyset$?($cd$.conjPaquetes) $\Leftrightarrow$
		vacía?($cd$.colaPaquetesDCNet)) $\yluego$ \\
	\- \- ($\neg$vacía?($cd$.colaPaquetesDCNet) $\implies$ \\
