...
'\alg{Copiar}{\In{r}{red}}{red}'
    ...
    while HaySiguiente?(itCompus) do '\ote{1}'
      auxnr $\leftarrow$ Obtener(r.dns, Siguiente(itCompus).ip)
      nr:nodoRed $\leftarrow$ <&Siguiente(itCompus) , Copiar (auxnr.caminos),
       CopiarConexiones(res.dns, auxnr.conexiones, Siguiente(itCompus).interfaces)> '\ote{1 + Copiar(caminos) + L}'
      Definir(res.dns, Siguiente(itCompus).ip, nr ) '\ote{L}'
      Avanzar(itCompus) '\ote{1}'
    end while  '\ote{n$^2$(L + n$^2$)}'
  '\ofi{O(n^2(L + n^2))}'
\end{lstlisting}
O(Copiar(caminos)) = O(n(L + Copiar(conj(lista(compu))))) \newline
O(Copiar(conj(lista(compu)))) = O(n$^2$) \newline
O(Copiar(caminos)) = O(n(L + n$^2$)) \newline
\begin{lstlisting}[mathescape]
  '\alg{CopiarConexiones}{\Inout{dns}{diccString(nodoRed)}, \In{conexs}{diccLineal(nat, puntero(nodoRed))}, 
    \In{interfaces}{conj(nat)} }{diccLineal (nat, puntero(nodoRed))}'
    res $\leftarrow$ Vacio()  '\ote{1}'
    itInterfs :itConj(nat) $\leftarrow$ CrearIt(interfaces)   '\ote{1}'
    while HaySiguiente?(itInterfs) do '\ote{1}'
      ip:string $\leftarrow$ Obtener(conexs,Siguiente(itInterfs)) '\ote{1}'
      if(!Definido?(dns,ip)) THEN '\ote{L}'
        Definir(dns,ip,<null,Vacia(),Vacio()>) '\ote{L}'
      end if '\ote{L}'
      Definir(res, Siguiente(itInterfs), &Obtener(dns,ip)) '\ote{L}'
