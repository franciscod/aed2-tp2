...
\tadAxioma{darCaminosMinimos($nA$, $nB$)}{
  ...
  \Abs[estr]{red}[e]{r}{
    ...
    (($\forall$ $c1$, $c2$: compu, $i1$, $i2$: string, $n1$, $n2$: nodoRed) ( \\
        \- \- ($c1$ $\in$ $e$.compus $\land$ $i1$ = $c1$.ip $\land$ $n1$ = obtener($i1$, $e$.dns) $\land$ $c1$ = $n1$.pc) $\land$ \\
        \- \- ($c2$ $\in$ $e$.compus $\land$ $i2$ = $c2$.ip $\land$ $n2$ = obtener($i2$, $e$.dns) $\land$ $c2$ = $n2$.pc) $\land$ \\
        \- \- ($c1$ $\neq$ $c2$))
        ) $\impluego$ \\
        ...
        \- \- \- \- ))) $\land$ ( \\
        \- \- \- \- \- \- caminosMinimos($r$, $c1$, $c2$) = obtener($i2$, $n1$.caminos) \\
        \- \- \- \- )
   }
