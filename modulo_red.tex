...
'\alg{CrearTodosLosCaminos}{\Inout{r}{red}}{}'
  ...
  while HaySiguiente?(itCompuA) do  '\ote{1}'
    ...
    while HaySiguiente?(itCompuB) do  '\ote{1}'
      ...
      caminimos:conj(lista(compu)) $\leftarrow$ Minimos(Caminos(nr, Siguiente(itCompuB).ip)
      Definir(nr.caminos, Siguiente(itCompuB).ip, caminimos)
...
'\alg{Caminos}{\In{c1}{nodoRed}, \In{ipDestino}{string}}{conj(lista(compu))}'
  ...
  iCandidatos:lista(nodoRed) $\leftarrow$ listaNodosVecinos(c1)
  iRecorrido:lista(compu) $\leftarrow$ Vacia()
  AgregarAdelante(iRecorrido, *(c1.pc))

  Apilar(frameRecorrido, iRecorrido)
  Apilar(frameCandidatos, iCandidatos)

  pCandidatos:compu
  fCandidatos:lista(nodoRed)

  while $\neg$EsVacia?(frameRecorrido) do  '\ote{1}'
    iRecorrido $\leftarrow$ Tope(frameRecorrido)
    iCandidatos $\leftarrow$ Tope(frameCandidatos)

    Desapilar(frameRecorrido)
    Desapilar(frameCandidatos)

    pCandidatos $\leftarrow$ Primero(iCandidatos)

    if $\neg$EsVacio?(iCandidatos) then
      Fin(iCandidatos)
      fCandidatos $\leftarrow$ iCandidatos

      if ult(iRecorrido).pc$\rightarrow$ip = ipDestino then
        AgregarRapido(res, iRecorrido)
      else
        Apilar(frameRecorrido, iRecorrido)
        Apilar(frameCandidatos, fCandidatos)

        if $\neg$nodoEnLista(pCandidatos, recorrido) then
          iRecorrido $\leftarrow$ Copiar(iRecorrido)
          AgregarAtras(iRecorrido, *(pCandidatos))
          Apilar(frameRecorrido, iRecorrido)
          Apilar(frameCandidatos, listaNodosVecinos(pCandidatos))
        fi
      fi
...
'\alg{Minimos}{\In{caminos}{conj(lista(compu))}}{conj(lista(compu))}'
  res $\leftarrow$ Vacio()
  longMinima:int
  itCaminos:itConj(lista(compu)) $\leftarrow$ CrearIt(caminos)
  if HaySiguiente?(itCaminos) then
    longMinima $\leftarrow$ Longitud(Siguiente(itCaminos))
    Avanzar(itCaminos)
    ...
    itCaminos $\leftarrow$ CrearIt(caminos)
    while HaySiguiente?(itCaminos)
      if Longitud(Siguiente(itCaminos)) = longMinima then
