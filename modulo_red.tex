...
\begin{lstlisting}[mathescape]
  '\alg{iAgregarComputadora}{\Inout{r}{red}, \In{c}{compu}}{}'
    ...
    DefinirRapido(r.dns, compu.ip, Tupla<&c,Vacio(),Vacio()>) '\ote{1}'
    GeneroCaminosDeCompuNueva(r,c) '\ote{n}''
  '\ofi{O(n*L)}'
\end{lstlisting}
...
\begin{lstlisting}[mathescape]
  '\alg{AgregoCompuNuevaAlResto}{\Inout{r}{red}, \In{c}{compu}}{}'
    ...
    while HaySiguiente?(itCompus) do                                                    
      ...
      DefinirRapido(nr.caminos, c.ip, Vacio()) '\ote{1}'
...
\begin{lstlisting}[mathescape]
  '\alg{GeneroCaminosDeCompuNueva}{\Inout{r}{red}, \In{c}{compu}}{}'
    ...
    while HaySiguiente?(itCompus) do                                                    
      DefinirRapido(cams, Siguiente(itCompus).ip, Vacio())  '\ote{1}'
      Avanzar(itCompus) '\ote{1}'
    end while '\ote{n}'
  '\ofi{O(n)}'
\end{lstlisting}
